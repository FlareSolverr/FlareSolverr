...
| download            | Optional, default false. When `true`, downloads the file at `url` using the browser fetch API. Returns a `download` object with `filename`, `mime`, and Base64-encoded `data` in the JSON response.                                                                                                   |
| returnRawFile       | Optional, default false. Requires `download: true`. When `true`, returns the file as a raw binary HTTP response with the correct `Content-Type` header instead of Base64 in JSON. Use HTTP status to detect success or failure.                                                                                             |
